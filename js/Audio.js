...
export class GameAudio {
	...
	init( camera ) {
		...
		loader.load( ASSETS_BASE + 'audio/engine.ogg', ( buffer ) => {
		...
		loader.load( ASSETS_BASE + 'audio/skid.ogg', ( buffer ) => {
		...
		loader.load( ASSETS_BASE + 'audio/impact.ogg', ( buffer ) => {
	...
	playExplosion() {
		...
		// Reuse impact sound at full volume for explosion
		this.playImpact( 6 );
	...
	playPickup() {
		...
		// Reuse impact sound at low pitch for pickup
		if ( ! this.unlocked || this.impactPool.length === 0 ) return;
		const sound = this.impactPool[ this.impactIndex ];
		this.impactIndex = ( this.impactIndex + 1 ) % this.impactPool.length;
		if ( sound.isPlaying ) sound.stop();
